...
class OrderDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Добавляем доступные блюда для модального окна
        context['menu_items'] = MenuItem.objects.filter(is_available=True).select_related('category')
        return context
...
class OrderCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.total_price = 0  # Будет пересчитано после добавления позиций
